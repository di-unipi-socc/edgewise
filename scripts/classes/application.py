...
class Application:
    ...
    def get_thing_instance_by_id(self, thing_instance_id):
        return next(
            (t for t in self.thing_instances if t.id == thing_instance_id), None
        )
    ...
    def get_instance_data_flow(self, instance_id):
        return [
            ...
            if df.source.id == instance_id or df.target.id == instance_id
        ]
    ...
    def get_data_flow(self, source_id, target_id):
        return next(
            (
                df
                for df in self.data_flows
                if df.source.id == source_id and df.target.id == target_id
            ),
            None,
        )
