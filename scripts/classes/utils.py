...
from swiplserver import PrologMQI, PrologThread, prolog_args
...
def start_prolog_thread():
    print("Starting Prolog MQI and Thread...")
    mqi = PrologMQI()
    prolog_thread = mqi.create_thread()
    return mqi, prolog_thread


def stop_prolog_thread(mqi: PrologMQI, prolog_thread: PrologThread):
    print("Stopping Prolog Thread and MQI...")
    prolog_thread.stop()
    mqi.stop()
