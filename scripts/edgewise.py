...
def init_parser() -> ap.ArgumentParser:
    ...
    p.add_argument(
        ...
        "--model",
        action="store_true",
        help="if set, stores the model in LP format.",
    ),
...
def get_compatibles(prolog: PrologThread, app_path, infr_path, app_name):
    prolog.query(f"consult('{app_path}')")
    prolog.query(f"consult('{infr_path}')")
    prolog.query(f"consult('{join(PL_UTILS_DIR, 'cr-engine.pl')}')")
...
def edgewise(
    ...
    compatibles = get_compatibles(prolog, app.get_file(), infr.get_file(), app.name)
    if not compatibles:
    ...
    if model:
        ...
        filename = join(
            MODELS_DIR,
            f'model_{app.name}_{infr.get_size()}{"_dummy" if dummy else ""}.lp',
        )
    ...
    if status == pywraplp.Solver.OPTIMAL:
        ...
        print(Fore.LIGHTWHITE_EX + f"Assert placement for {app.name}: {str_pl}\n")
    else:
