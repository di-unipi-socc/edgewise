...
bestPlacement(App, BestPlacement, BestCost, CapCost) :-
    findall((C, P), placement(App, P, C, CapCost), Ps),
    sort(1, @=<, Ps, Sorted), nth0(0, Sorted, (BestCost, BestPlacement), _).

placement(App, Placement, Cost, CapCost) :-
    application(App, Functions, Services),
    functionPlacement(Functions, FPlacement, FCost),
    servicePlacement(Services, FPlacement, SPlacement, SCost),
    Placement = (FPlacement, SPlacement),
    qosOK(Placement),
    Cost is FCost + SCost, Cost < CapCost.
...
functionPlacement([F|Fs], [(F,N,HWReqs)|Ps], NewCost) :-
    ...
    cost(NType, F, C), NewCost is Cost + C.
functionPlacement([], [], 0).

servicePlacement([S|Ss], FPlacement, [(S,N,HWReqs)|Ps], NewCost) :-
    servicePlacement(Ss, FPlacement, Ps, Cost),
    ...
    cost(NType, S, C), NewCost is Cost + C.
servicePlacement([], _, [], 0).
