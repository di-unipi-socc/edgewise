...
dataFlow(soil, snsQueue, snsLevel, [enc], 0.04, 100, 700).
dataFlow(heat, snsQueue, snsLevel, [enc], 0.04, 100, 700).
dataFlow(water, snsQueue, snsLevel, [enc], 0.04, 80, 700).
dataFlow(nutrient, ctrlQueue, ctrlLevel, [enc], 0.08, 50, 700).
dataFlow(energy, ctrlQueue, ctrlLevel, [enc], 0.08, 50, 700).
dataFlow(piCamera1, imgQueue, picture, [enc], 0.1, 100, 1000).
dataFlow(piCamera2, imgQueue, picture, [enc], 0.1, 100, 1000).

dataFlow(snsQueue, mainDB, snsLevel, [enc], 0.04, 90, 700).
dataFlow(ctrlQueue, mainDB, ctrlLevel, [enc], 0.08, 50, 700).
dataFlow(imgQueue, mainDB, picture, [enc], 0.1, 100, 2000).

dataFlow(imgQueue, calibre, picture, [enc], 0.1, 100, 300).
dataFlow(calibre, piCamera1, calibrationData, [enc], 0.01, 50, 300).
dataFlow(calibre, piCamera2, calibrationData, [enc], 0.01, 50, 300).

dataFlow(imgQueue, rectify, picture, [enc], 0.1, 150, 300).
dataFlow(rectify, roiSel, rectPicture, [enc], 0.2, 100, 400).
dataFlow(rectify, recognizer, rectPicture, [enc], 0.2, 100, 400).
dataFlow(calibre, recognizer, calibrationData, [enc], 0.01, 50, 300).
dataFlow(roiSel, recognizer, regionData, [enc], 0.02, 50, 400).

dataFlow(recognizer, visualizer, projectionData, [enc], 0.02, 100, 50).
dataFlow(imgQueue, visualizer, picture, [enc], 0.1, 100, 50).
dataFlow(visualizer, arViewer, info, [enc], 0.2, 90, 60).