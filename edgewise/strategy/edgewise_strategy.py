...
class EdgeWiseStrategy(PlacementStrategy):
    ...
    def place(
        ...
        timed_query(self.prolog, "make")

        components_mapping, self.cost, exec_time_1 = self.exec(
            application, infrastructure, cr=self.cr
        )

        exec_time_2 = None
        if components_mapping == {} and self.cr:  # retry without CR if failed
            components_mapping, self.cost, exec_time_2 = self.exec(
                application, infrastructure, cr=False
            )

        self.exec_time = (exec_time_1 or 0) + (exec_time_2 or 0)
            ...
            self.moved_services = (
                (
                    len(
                        [
                            s
                            for s, n in components_mapping.items()
                            if self.last_valid_mapping.get(s, "") not in ["", n]
                        ]
                    )
                )
                if self.last_valid_mapping is not None
                else 0
            )
            self.last_valid_mapping = components_mapping
