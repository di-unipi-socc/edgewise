...
dataFlow(iphoneXS, uploadPost, post, [enc_iot, authentication], 0.4, 5, 60).
...
dataFlow(convertTxt, echoDot, speech, [enc_iot, access_logs], 0.5, 8, 30).
dataFlow(convertTxt, metaAudio, speech, [authentication, enc_storage], 0.5, 8, 70).
dataFlow(metaAudio, mainDB, meta, [anti_tampering, enc_storage], 0.1, 10, 40).
dataFlow(convertTxt, uploadAudio, speech, [access_logs], 0.8, 8, 30).
